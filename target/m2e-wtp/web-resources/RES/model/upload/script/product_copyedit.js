...
function initAttr(){
	...
	$.mypost({
		 ...
		 callback:function(result){
			 ...
			 initSkuData(result.marketAttrs);
		 }
	 });
}
/**
 * 获取sku
 * */
function initSkuData(marketAttrs){
	var params = new Object();
	params.spuId = spuId;
	$.mypost({
		params:params,
...
function getRowMarketAttrHtml(row,i){
	...
	var addOptionBtnHtml ='<label class="checkbox_li"><span class="inline deco new_box" data-count="-1" data-index="'+i+'">增加自定义选项</span></label>';
