    ...
    var plugins = {
            ...
            pager: {
                "default": function( page, lastpage, pagerOptions ) {
                    ...
                    var page_size = datagrid._params.paging;
                    var start_count = (datagrid._params.page-1)*page_size+1;
                    var end_count   = datagrid._params.page*page_size;
                    
                    if(end_count>datagrid._params.total){
                    	end_count = datagrid._params.total;
                    }
                    
                    var pageSizeSelContainer = $('<label class="ma_r10">'
						                        +'<select name="" id="'+pluginName+'_page_size" aria-controls="sample_2" class="form-control input-xsmall input-inline pageSelect" title="">'
						                        ...
						                        +'</select> 行/页'
						                        +'</label>'
						                        +'<span class="inline">第'+start_count+'-'+end_count+'行 共'+datagrid._params.total+'行</span>');
                    
                    pageSizeSelContainer.find("#"+pluginName+"_page_size").val(page_size);
                    
//                    console.log(pageSizeSelContainer.html())
                    
                    var pageWrapperOuter = $('<div class="row"><div class="col-md-5 col-sm-12"><div class="dataTables_length" id="sample_2_length"></div></div><div class="col-md-7 col-sm-12"><div class="dataTables_paginate paging_simple_numbers f_r" id="sample_editable_1_paginate"></div></div></div>');
                    pageWrapperOuter.find("#sample_editable_1_paginate").append(container);
                    pageWrapperOuter.find("#sample_2_length").append(pageSizeSelContainer);
                    
                    return pageWrapperOuter[0];
                }
            }
        };
    ...
    Plugin.prototype = {
        init: function () {
            ...
            this.$el.on("change","#"+pluginName+"_page_size",function(e){
            	e.preventDefault();
                datagrid.page(1);
                datagrid.paging($("#"+pluginName+"_page_size").val());
                datagrid.fetch();
            });
